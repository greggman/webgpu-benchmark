...
export async function initGpu(canvas: HTMLCanvasElement): Promise<GpuInit> {
  ...
  const requiredFeatures: GPUFeatureName[] = hasTimestamp ? ['timestamp-query'] : [];

  const device = await adapter.requestDevice({ requiredFeatures });

  const context = canvas.getContext('webgpu');
  if (!context) throw new GpuUnsupportedError('canvas.getContext("webgpu") returned null');
  ...
  return { adapter, device, context, canvas, format, hasTimestamp, adapterInfo: adapter.info };
}
